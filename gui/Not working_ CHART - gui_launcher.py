...
# ----------------------------------------------------
# Call a job safely (GUI compatible)
# ----------------------------------------------------
def call_job(module_obj: Any, func_name: str, enqueue: callable) -> Any:
    ...
    try:
        ...
        # only pass gui_enqueue if supported
        if "gui_enqueue" in sig.parameters:
            result = func(gui_enqueue=enqueue)
        else:
            result = func()
...
class LaundryApp(ctk.CTk):
    def __init__(self):
        ...
        # Chart results storage
        self.job_results_chart: List[float] = []
        self.job_labels_chart: List[str] = []
    ...
    def _create_main_area(self):
        ...
        # Output textbox
        self.output_box = ctk.CTkTextbox(right, width=600, height=200)
        self.output_box.pack(fill="both", expand=False, padx=8, pady=(0, 8))
        self.output_box.configure(state="disabled")

        # Matplotlib chart frame
        self.fig, self.ax = plt.subplots(figsize=(6,2))
        self.ax.set_title("Job Trend Chart")
        self.ax.set_xlabel("Job #")
        self.ax.set_ylabel("Value / Result")
        self.ax.grid(True)
    ...
    def _append_output(self, line: str, tag: Optional[str] = None):
        ...
        elif tag == "succ" or line.startswith("[INFO]") or line.startswith("[RESULT]") or line.startswith("[SUCCESS]"):
            self.output_box.insert("end", line + "\n")
        else:
            self.output_box.insert("end", line + "\n")
    ...
    def update_chart(self, value: float, label: str = ""):
        self.job_results_chart.append(value)
        self.job_labels_chart.append(label if label else str(len(self.job_results_chart)))
        self.ax.clear()
        self.ax.bar(range(len(self.job_results_chart)), self.job_results_chart, color="green")
        self.ax.set_xticks(range(len(self.job_results_chart)))
        self.ax.set_xticklabels(self.job_labels_chart, rotation=45, ha="right")
        self.ax.set_title("Job Trend Chart")
        self.ax.set_xlabel("Jobs")
        self.ax.set_ylabel("Value / Result")
        self.ax.grid(True)
        self.canvas.draw()
        ...
        for j in self.jobs:
            ...
            def run_job(mod=mod, fn=cb):
                def inner():
                    ...
                    # auto-update chart if numeric result
                    if isinstance(result, (int, float)):
                        self.update_chart(result, label=mod.__name__)
